...
const availableAmount = 450.0;
const minPayout = 100.0;

const payoutMethods: PayoutMethod[] = [
  {
    id: 'pm1',
    type: 'Bank Transfer',
    details: 'Bank of Valletta, IBAN: MT...1234',
    isDefault: true
  },
  {
    id: 'pm2',
    type: 'Revolut',
    details: 'Username: @maria123',
    isDefault: false
  }
];
...
const payoutHistory: PayoutHistoryItem[] = [
  ...
  },
  {
    ...
    amount: 650.0,
    status: 'Paid',
    paidOn: new Date('2023-09-04'),
    method: 'Bank Transfer'
  },
  {
    id: 'ph3',
    requestDate: new Date('2023-08-01'),
    amount: 520.5,
    status: 'Paid',
    paidOn: new Date('2023-08-03'),
    method: 'Revolut'
  }
];
...
export function PayoutsPage() {
  ...
  const handleRequestPayout = () => {
    toast.success(
      `Payout request for €${availableAmount.toFixed(2)} has been submitted!`
    );
  };
    ...
    <div className='space-y-8 p-4 md:p-6 lg:p-8'>
      <header>
        ...
        <p className='text-muted-foreground'>
          Manage your payout methods and view your payment history.
        </p>
      </header>
      ...
      <Card>
        ...
        <CardContent className='space-y-4'>
          <Alert>
            ...
            <AlertTitle>
              Available for Payout: €{availableAmount.toFixed(2)}
            </AlertTitle>
            <AlertDescription>
              The minimum amount for a payout request is €{minPayout.toFixed(2)}
              .
            </AlertDescription>
          </Alert>
          <Button
            variant='outline'
            size='lg'
            className='w-full not-dark:text-red-400 sm:w-auto'
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select your default method for receiving payments.
          </CardDescription>
        </CardHeader>
        <CardContent className='space-y-4'>
          <RadioGroup defaultValue='pm1' className='space-y-2'>
            {payoutMethods.map((method) => (
              <Label
                key={method.id}
                htmlFor={method.id}
                className='hover:bg-accent hover:text-accent-foreground [&:has([data-state=checked])]:border-primary flex flex-col items-start gap-4 rounded-md border p-4 sm:flex-row sm:items-center'
              >
                <RadioGroupItem value={method.id} id={method.id} />
                <div className='flex items-center gap-3'>
                  {method.type === 'Bank Transfer' && (
                    <Landmark className='text-muted-foreground h-6 w-6 not-dark:text-red-400' />
                  )}
                  {method.type === 'Revolut' && (
                    <Wallet className='text-muted-foreground h-6 w-6 not-dark:text-red-400' />
                  )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A record of all payouts requested and sent to you.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Desktop Table */}
              ...
              <TableBody>
                {payoutHistory.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>
                      {format(item.requestDate, 'dd MMM yyyy')}
                    </TableCell>
                    ...
                    <TableCell>
                      {item.paidOn ? format(item.paidOn, 'dd MMM yyyy') : 'N/A'}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
              ...
              <Card key={item.id}>
                ...
                <CardContent className='text-muted-foreground space-y-1 text-sm'>
                  <p>
                    ...
                    {format(item.requestDate, 'PPP')}
                  </p>
                  <p>
                    <strong>Paid On:</strong>{' '}
                    {item.paidOn ? format(item.paidOn, 'PPP') : 'N/A'}
                  </p>
                </CardContent>
              </Card>
            ))}
